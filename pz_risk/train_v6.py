...
class Coach:
    ...
    def initialize(self):
        self.env = env(n_agent=self.n_agents, board_name='r_{}_{}_random'.format(self.n_nodes, self.n_nodes * 2))
        ...
        # self.env = wrappers.SparseRewardWrapper(self.env)
        self.env.reset()
        data = self.env.last()[0]['data']
        self.feat_size = self.env.observation_spaces['feat'].shape[0]
        self.type_size = self.n_nodes
        self.critic = DVNAgent(self.n_nodes, self.n_agents, self.feat_size, self.hidden_size, data, self.device)
        self.last_critic = DVNAgent(self.n_nodes, self.n_agents, self.feat_size, self.hidden_size, data, self.device)
        if self.episode == 0:
    ...
    def gather_experience(self, _):
        ...
        for i, agent_id in enumerate(self.env.agent_iter(max_iter=self.args.num_steps)):
            ...
            self.critic.save_memory(transition)

            if all(state['dones']):
                print('done')
                break

        return self.critic.memory.memory
