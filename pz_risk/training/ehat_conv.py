...
class EHATConv(MessagePassing):
    ...
    def __init__(self, in_channels: int, out_channels: int,
        ...
        self.edge_attr_emb = Linear(3 * out_channels + edge_type_emb_dim + edge_dim, edge_attr_emb_dim, bias=False)

        self.att = Linear(
            3 * out_channels + edge_type_emb_dim + edge_attr_emb_dim,
    ...
    def message(self, x_i: Tensor, x_j: Tensor, edge_type_emb: Tensor,
        ...
        edge_pre_attr = torch.cat([x_i, x_j, x_i + x_j, edge_type_emb, edge_attr], dim=-1)
        edge_attr = F.leaky_relu(self.edge_attr_emb(edge_pre_attr), self.negative_slope)
        self.__edgemat__ = edge_attr
        alpha = torch.cat([x_i, x_j, x_i + x_j, edge_type_emb, edge_attr], dim=-1)
