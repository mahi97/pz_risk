...
class DVNAgent(nn.Module):
    def __init__(self, num_nodes, num_agents, node_feat_size, hidden_size, device='cuda:0'):
        ...
        self.device = device
        self.policy_network = HATGNN(node_feat_size, 0, hidden_size).to(device)
        self.target_network = HATGNN(node_feat_size, 0, hidden_size).to(device)
    ...
    def get_value_and_probs(self, board, agent_id):
        ...
        p = None
        if board.state == GameState.Reinforce:
            p = x[data.reinforce_index].squeeze().detach().cpu().numpy()
        elif board.state == GameState.Card:
            p = [0, 1]
        elif board.state == GameState.Attack:
            p = np.insert(e[data.attack_index].squeeze().detach().cpu().numpy(), 0, 0.1)
        elif board.state == GameState.Move:
            p = torch.sigmoid(e[data.move_index].squeeze()).detach().cpu().numpy()
        elif board.state == GameState.Fortify:
    ...
    def predict(self, board, agent_id):
        ...
        v, probs = self.get_value_and_probs(board, agent_id)
